...
API_KEY = { "popular": "04bd8af1d12f4a1e9ee290a80e694c34",
            "article": "04bd8af1d12f4a1e9ee290a80e694c34"}

# look for JSON entries of a URL king of 'viewed' for a period of 1 day.
...
def article_overview(kind, period):
    ...
    titles = []
    urls =[]
    # Create a list of dictionaries where the key is the return from the "section" query and the value is the return from the "title" query
    for entry in data:
        #extract section and title values from data title
        section = entry['section']
        title = entry['title']

        #assign extract values to become the name value pairing of "section: title"
        titles.append({section:title})

         #source the media attributes of URL and Format
         # multiple loops required for every layer of JSON element required to obtain URL
        if "media" in entry:
    	    for m in entry['media']: #level 2
    		    for mm in m['media-metadata']: #level 3
    			    if mm["format"]== "Standard Thumbnail":  #only take JSON elements in Standard Thumbnail format
    			        urls.append(mm['url'])
    			        #print urls
